...
// Create Hong Kong time and convert to UTC properly
const createHongKongTimeAsUTC = (dateStr: string, timeStr: string): Date => {
  const eventDate = parseISO(dateStr);
  const [hours, minutes] = timeStr.split(':').map(Number);
  
  // Create the local Hong Kong time
  const hkTime = new Date(eventDate);
  hkTime.setHours(hours, minutes, 0, 0);
  
  // Convert Hong Kong time to UTC using proper timezone handling
  return zonedTimeToUtc(hkTime, HK_TIMEZONE);
};
...
export const openGoogleCalendarEvents = (events: Event[], userAgenda: Record<string, string | undefined>): void => {
  ...
  eventsToOpen.forEach((event, index) => {
    ...
    const startDateTime = createHongKongTimeAsUTC(event.date, arrivalTime);
...
export const generateICSContent = (events: Event[], userAgenda: Record<string, string | undefined>): string => {
  ...
  events.forEach((event, index) => {
    ...
    // Use proper Hong Kong timezone conversion
    const startDateTime = createHongKongTimeAsUTC(event.date, arrivalTime);
